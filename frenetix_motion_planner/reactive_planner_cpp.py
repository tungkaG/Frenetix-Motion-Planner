...
from frenetix_motion_planner.trajectory_board import TrajectoryBoard, CartesianSample
...
class ReactivePlannerCpp(Planner):
    ...
    def setup_subscriber(self, client):
        ...
        def on_msg(msg):
            ...
            for i, sample in enumerate(msg["samples"]):
                traj = TrajectoryBoard()

                # Fill cartesian sample as numpy arrays
                traj.cartesian.x = np.array(sample["x"], dtype=float)
                traj.cartesian.y = np.array(sample["y"], dtype=float)
                traj.cartesian.theta = np.array(sample["theta"], dtype=float)
                traj.cartesian.v = np.array(sample["velocity"], dtype=float)
                traj.cartesian.acceleration = np.array(sample["acceleration"], dtype=float)
                traj.cartesian.a = traj.cartesian.acceleration   # alias for convenience
                traj.cartesian.kappa = np.array(sample["kappa"], dtype=float)
                traj.cartesian.kappa_dot = np.array(sample["kappa_dot"], dtype=float)

                curv = msg["samples_curv"][i]
                traj.curvilinear.s   = np.array(curv["s"], dtype=float)
                traj.curvilinear.s_dot  = np.array(curv["ss"], dtype=float)
                traj.curvilinear.s_ddot = np.array(curv["sss"], dtype=float)
                traj.curvilinear.d   = np.array(curv["d"], dtype=float)
                traj.curvilinear.d_dot  = np.array(curv["dd"], dtype=float)
                traj.curvilinear.d_ddot = np.array(curv["ddd"], dtype=float)

                # Cost and feasibility arrays align by index
                if i < len(msg["cost"]):
                    traj.cost = float(msg["cost"][i])
                if i < len(msg["feasibility"]):
                    traj.feasible = bool(msg["feasibility"][i])
    ...
    def plan(self) -> tuple:
        ...
        # =============================================================
        # Subscribe feasible trajectory from board
        while (not self.received_board_trajectory):
            time.sleep(0.2)
        
        self.received_board_trajectory = False

        # Take only feasible trajectories
        feasible_trajectories_board = [t for t in self.trajectories_board if t.feasible]

        # Sort them by cost
        sorted_trajectories_board = sorted(feasible_trajectories_board, key=lambda t: t.cost)

        # collision check
        optimal_trajectory_board = self.trajectory_collision_check(sorted_trajectories_board)

        if optimal_trajectory_board is None:
            optimal_trajectory_board = optimal_trajectory
            self.msg_logger.warning("No optimal trajectory for BOARD available. Select trajectory from HPC!")
