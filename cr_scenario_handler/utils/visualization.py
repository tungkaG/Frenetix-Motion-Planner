...
def visualize_agent_at_timestep(scenario: Scenario, planning_problem: PlanningProblem, ego: DynamicObstacle,
                                timestep: int, config, log_path: str, traj_set=None, optimal_traj=None, optimal_traj_board=None,
    ...
    # GLENN: X and Y is state.position[0] and [1]
    if optimal_traj:
    ...
    if optimal_traj_board:
        optimal_traj_positions_board = np.array([(state.position[0], state.position[1]) for state in optimal_traj_board.state_list[0+replanning_counter:]])
        rnd.ax.plot(optimal_traj_positions_board[:, 0], optimal_traj_positions_board[:, 1], color="purple", marker="o", markersize=1.5, zorder=21, linewidth=2.0)
